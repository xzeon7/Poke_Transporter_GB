...
void init_payload(byte *payload_buffer, const GB_ROM &curr_rom, int type, bool debug)
{
    ...
    
    {
        ...
        */
        ...
        removal_array.load_data(41, removal_array_data);
        z80_variable transfer_wait_string(&var_vector, 13, // SENDING DATA　そうしんちゅう
                                          0x92, 0x84, 0x8D, 0x83, 0x88, 0x8D, 0x86, 0x7F, 0x83, 0x80, 0x93, 0x80, 0x50);
        z80_variable custom_name(&var_vector, 11, // FENNEL フェネル
                                 0x85, 0x84, 0x8D, 0x8D, 0x84, 0x8B, 0x50, 0x50, 0x50, 0x50, 0x50);
        ...
        //distance -= 8;   No -8 adjustmant needed.
    // *** KEY DIFFERENCE FROM ENG CRYSTAL ***
    // Distance is inverted because stack is BELOW print_string_start
    // due to mobile adapter code pushing SP down to 0xC0BB
        for (int i = 0; i < distance; i++)
        {
            z80_payload.add_byte(curr_rom.short_pkmn_name); //0x4E
        }
